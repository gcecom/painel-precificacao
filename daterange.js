...
// "YYYY-MM" de n dias atrás (para "últimos X dias"), local
function ymDaysAgo(n){const d=new Date();d.setDate(d.getDate()-n);return ymOf(d)}
...
const brDate=s=>/^\d{4}-\d{2}-\d{2}$/.test(s||'')?s.slice(8,10)+'/'+s.slice(5,7)+'/'+s.slice(0,4):'';

// Presets de período do Dashboard (dados são mensais → cada preset vira um intervalo de meses)
const RANGE_PRESETS=[
  {k:'hoje',  label:'Hoje',             range:()=>[thisMonth(),thisMonth()]},
  {k:'7d',    label:'Últimos 7 dias',   range:()=>[ymDaysAgo(6),thisMonth()]},
  {k:'30d',   label:'Últimos 30 dias',  range:()=>[ymDaysAgo(29),thisMonth()]},
  {k:'mes',   label:'Este mês',         range:()=>[thisMonth(),thisMonth()]},
...
function mountRange(hostId,fromId,toId){
  ...
  let lastKey='30d',customDates=null;
  ...
  cust.innerHTML='<label>Data inicial</label><input type="date" data-cf>'
               +'<label>Data final</label><input type="date" data-ct>'
  ...
  function toggleCustom(){cust.classList.toggle('show')}
  cust.querySelector('.dp-apply').onclick=()=>{
    if(!cf.value||!ct.value)return;
    const a=cf.value<=ct.value?cf.value:ct.value,b=cf.value<=ct.value?ct.value:cf.value; // ordena sem parse UTC
    applyRange(a.slice(0,7),b.slice(0,7),{key:'custom',a,b});                             // fatia local -> mês
  };
  ...
  function setLabel(){
    ...
    if(lastKey==='custom'&&customDates){txt=brDate(customDates.a)+' – '+brDate(customDates.b)}
    else{
  ...
  function ensureDefault(){ // padrão: Últimos 30 dias quando não há período definido
    if(el(fromId)&&el(toId)&&(!el(fromId).value||!el(toId).value)){
      const[f,t]=RANGE_PRESETS.find(p=>p.k==='30d').range();
      el(fromId).value=f;el(toId).value=t;lastKey='30d';customDates=null;
    }
  }
  ensureDefault();setLabel();
  registry[fromId]={refresh:()=>setLabel()};

  // Reaplica o padrão e mantém o rótulo em sincronia mesmo depois de resetDashboard()
  // limpar os campos (não altera cálculo: só garante o período e atualiza o texto do botão).
  const orig=window.renderDashboard;
  if(typeof orig==='function'){
    window.renderDashboard=function(){ensureDefault();const r=orig.apply(this,arguments);setLabel();return r};
  }
}
...
// Monta os dois seletores (os elementos já existem no HTML estático)
mountRange('dashPeriod','dashFrom','dashTo');
